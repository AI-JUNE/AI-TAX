...
// ✅ 서버(Vercel)를 통해 API 호출 - API 키 브라우저 노출 없음
async function callClaude(messages, system) {
  ...
  if (!cleaned.length) throw new Error("전송할 메시지가 없습니다.");

  const res = await fetch("/api/chat", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ messages: cleaned, system }),
  });
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) throw new Error("요청 한도 초과 — 잠시 후 재시도해주세요.");
...
※ 3만원 초과 증빙미비·주말·심야(22시 이후) 자동 🔴`,
  ...
  },
  {
    ...
    sys: `[Tax Calculator 모드] 2026년 세율 단계별 계산. 모든 산식 표시. 마크다운 표로 결과 요약.`,
  },
  {
    ...
    sys: `[Schedule 모드] 2026년 기한 기준 표 형식 월별 정리. 법인/개인사업자 구분. 가산세 위험 고지.`,
  },
  {
...
const readFile = f => new Promise((res, rej) => {
  const r = new FileReader(); r.onload = e => res(e.target.result); r.onerror = () => rej(new Error("파일 읽기 실패")); r.readAsText(f, "UTF-8");
});
...
export default function App() {
  ...
  const send = useCallback(async (override) => {
    ...
    try {
      const reply = await callClaude(next, BASE_SYS + "\n\n" + mod.sys);
  ...
  const onFile = e => {
    ...
    if (f.size > 5 * 1024 * 1024) { toast("파일 크기는 5MB 이하만 가능합니다."); return; }
    if (!/\.(csv|txt|tsv|json)$/i.test(f.name)) { toast("CSV, TXT, TSV, JSON 형식만 지원합니다."); return; }
    ...
    <>
        ...
        <aside className={`sb ${drawer ? "sb-open" : ""}`}>
          <div className="sb-head">
            <div className="logo">
              ...
              <div><div className="logo-name">세무 어시스턴트</div><div className="logo-ver">AI · 2026 세법 기준</div></div>
            </div>
          </div>
          <nav className="sb-nav">
            ...
            {MODS.map(m => {
              const on = modId === m.id;
              return (
                <button key={m.id} className={`nav-btn ${on ? "nav-on" : ""}`}
                  style={on ? { background: m.light, borderColor: m.mid } : {}}
                  onClick={() => switchMod(m.id)}>
                  <span className="nav-ico">{m.icon}</span>
                  <div className="nav-txt">
                    <span className="nav-lbl" style={on ? { color: m.color } : {}}>{m.label}</span>
                    <span className="nav-tag">{m.tag}</span>
                  </div>
                  {on && <span className="nav-pip" style={{ background: m.color }} />}
                </button>
              );
            })}
          </nav>
          <div className="sb-foot"><span className="gdot" /><span className="sb-stat">Claude Sonnet 4</span></div>
        </aside>

        <div className="main">
          <header className="hdr">
            <button className="hbg" onClick={() => setDrawer(v => !v)}><span /><span /><span /></button>
            <div className="hdr-pill" style={{ background: mod.light, borderColor: mod.mid, color: mod.color }}>
              {mod.icon}<span className="hdr-pill-lbl">{mod.label}</span>
            </div>
          ...
          <div className="tabbar">
            {MODS.map(m => {
              const on = modId === m.id;
              return (
                <button key={m.id} className={`tabpill ${on ? "tabpill-on" : ""}`}
                  style={on ? { background: m.light, color: m.color, borderColor: m.mid } : {}}
                  onClick={() => switchMod(m.id)}>
                  {m.icon} {m.label}
                </button>
              );
            ...
            {msgs.length === 0 ? (
              <div className="welcome">
                <div className="w-hero">
                  <div className="w-ico" style={{ background: mod.light, border: `2px solid ${mod.mid}`, color: mod.color }}>{mod.icon}</div>
                  <div className="w-title">{mod.label}</div>
                  <div className="w-desc">{mod.desc}</div>
                  <div className="w-chip">{mod.hint}</div>
                </div>
                <div className="w-qs">
                  <div className="qs-lbl">빠른 시작</div>
                  {mod.quick.map((q, i) => (
                    <button key={i} className="qcard"
                      style={{ "--qc": mod.color, "--qb": mod.light, "--qbr": mod.mid }}
                      onClick={() => send(q)} disabled={busy}>
                      <span className="qcard-n" style={{ color: mod.color }}>{i + 1}</span>
                      <span className="qcard-t">{q.split("\n")[0]}</span>
                      <span className="qcard-a" style={{ color: mod.color }}>›</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="msglist">
                {msgs.map(msg => (
                  <div key={msg.id} className={`mrow ${msg.role === "user" ? "mrow-u" : "mrow-ai"}`}>
                    <div className={`mav ${msg.role === "user" ? "mav-u" : "mav-ai"}`}
                      style={msg.role === "assistant" ? { background: mod.light, color: mod.color, borderColor: mod.mid } : {}}>
                      {msg.role === "user" ? "나" : mod.icon}
                    </div>
                    <div className={`mwrap ${msg.role === "user" ? "mwrap-u" : ""}`}>
                      <div className="mmeta">
                        <span className="msnd">{msg.role === "user" ? "담당자" : "AI 세무 어시스턴트"}</span>
                        <span className="mtm">{fmt(msg.time)}</span>
                      </div>
                      <div className={`mbub ${msg.role === "user" ? "mbub-u" : "mbub-ai"}`}
                        style={msg.role === "user" ? { background: mod.light, borderColor: mod.mid } : {}}>
                        {msg.role === "assistant"
                          ? <div className="mdwrap" dangerouslySetInnerHTML={{ __html: mdRender(msg.display) }} />
                          : <div className="utxt">{msg.display}</div>}
                      </div>
                      {msg.role === "assistant" && (
                        <button className="cpbtn" onClick={() => copyText(msg.id, msg.display)}>
                ...
                {busy && (
                  <div className="mrow mrow-ai">
                    <div className="mav mav-ai" style={{ background: mod.light, color: mod.color, borderColor: mod.mid }}>{mod.icon}</div>
                    <div className="mwrap">
                      <div className="mmeta"><span className="msnd">AI 세무 어시스턴트</span></div>
                      <div className="mbub mbub-ai">
                        <div className="typing"><span style={{ background: mod.color }} /><span style={{ background: mod.color }} /><span style={{ background: mod.color }} /></div>
                        <span className="typing-lbl">분석 중...</span>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div className="inpzone">
            {file && (
              <div className="fchip">
                <span>📎</span><span className="fn">{file.name}</span>
                <span className="fsz">({(file.size / 1024).toFixed(0)}KB)</span>
                <button className="fdel" onClick={() => setFile(null)}>✕</button>
              </div>
            )}
            <div className="ibox" style={{ "--ac": mod.color }}>
              ...
              <button className="iatt" onClick={() => fileRef.current?.click()}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48" />
                </svg>
              </button>
              <textarea ref={taRef} className="ita" rows={1}
                placeholder={mod.ph} value={text}
                onChange={e => { setText(e.target.value); resizeTA(); }}
                onKeyDown={onKey} disabled={busy} />
              <button className="sendbtn"
                style={{ background: (busy || (!text.trim() && !file)) ? "#CBD5E1" : mod.color }}
                onClick={() => send()} disabled={busy || (!text.trim() && !file)}>
            ...
            <div className="ihint">
              <span>Enter 전송 · Shift+Enter 줄바꿈 · CSV/TXT 첨부 가능</span>
              {text.length > 100 && <span style={{ color: text.length > 9000 ? "#DC2626" : "#94A3B8" }}>{text.length.toLocaleString()} / 10,000</span>}
            </div>
          </div>
        </div>
      </div>
      {err && (
        <div className="toast" onClick={() => setErr("")}>
          <span>⚠️</span><span className="toast-msg">{err}</span><span style={{ opacity: .5, marginLeft: 4 }}>✕</span>
        </div>
      )}
    </>
  );
...
body{background:#F1F5F9;font-family:'Noto Sans KR',-apple-system,sans-serif;color:#0F172A;-webkit-font-smoothing:antialiased}
